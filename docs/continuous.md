...
A particle in a potential, on a grid. The state is a wavefunction ψ(x) sampled at points, and the Hamiltonian is a differential operator discretised by finite differences.

Solves the 1D Schrödinger equation numerically, computes eigenstates, and evolves wavepackets in time. Potentials and their spectra generalise to any number of dimensions when the system is separable.

For spins, qubits and two-level systems — where the state is a vector and the Hamiltonian is already a matrix — see [discrete systems](discrete.md).
